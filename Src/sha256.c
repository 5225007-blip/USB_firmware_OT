/**
 * @file    sha256.c
 * @brief   Standard SHA-256 implementation (FIPS 180-4), portable C.
 */
...
static void sha256_transform(sha256_ctx_t *ctx, const uint8_t block[64])
{
    ...
    e = ctx->state[4]; f = ctx->state[5]; g = ctx->state[6]; h_ = ctx->state[7];

    for (int i = 0; i < 64; i++) {
        t1 = h_ + BSIG1(e) + CH(e, f, g) + K[i] + w[i];
    ...
    ctx->state[4] += e; ctx->state[5] += f; ctx->state[6] += g; ctx->state[7] += h_;
}
...
void SHA256_Final(sha256_ctx_t *ctx, uint8_t digest[32])
{
    ...
    /* Pad: 0x80, then zeros, leaving 8 bytes for the length field. */
